\chapter{Application on the Laplace problem}
\label{ch:results}
\glsresetall
...
\begin{align}
\Omega &= \left\{ (r,\theta) \in \mathbb{R}_{\geq 0} \times [0, 2 \pi) : 0 \leq \theta \leq \pi/\alpha \right\} \,\text{,}
\end{align}
...
\caption[Solution of the manufactured Laplace problem on the L-shaped domain.]{Solution from Eq.~(\ref{eq:ficherasolution}) of the manufactured Laplace problem on the L-shaped domain described by Eq.~(\ref{eq:Ldomain}).}
\label{fig:solution}
\end{figure}

...
For machines with shared memory access, \glsdesc{openmp}\glsunset{openmp} \autocite{openmp50} and \glsdesc{tbb}\glsunset{tbb} \autocite{tbb2018} with its work stealing policy are the most prominent approaches. IF architecture is distributed on nodes and thus have distributed memory access, the \glsdesc{mpi}\glsunset{mpi} \autocite{mpi31} will be used. A combination of both is possbile.

Further, recently streaming multiprocessor architecture have become more and more interest, that work on graphic accelerator cards (GPUS). \glsdesc{openacc}\glsunset{openacc} \autocite{openacc27} or nVidia's \glsdesc{cuda}\glsunset{cuda} \autocite{cuda10} can be used for that.

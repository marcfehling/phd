...
In general, \gls{fem} requires a subdivision of the domain $\Omega$ into smaller cells $K$, where $\cup_{i} K_i = \Omega$\todo{make this beautiful} and each cell $K$ corresponds to a connected Lipschitz domain with nonempty interior \todo{Check Ciarlet definition?}.
...
With this definition, we are able to find a set of basis functions ${\phi_1, \dots, \phi_n}$ of $P$ dual to $N$ fulfilling $\Psi_i(\phi_j) = \delta_{ij}$. In other words, these basis functions span\todo{Is this the right word?} the function space $P$ while each of them has the value one on their associated node and zero on the other.

...
According to Eq.~(\ref{eq:errorbound_hp}), we notice that a successfull refinement with \p-adaptation requires the solution to be reasonably regular.

Thus, the smoothness of a finite element approximation would be a reasonable indicator to decide between \h- and \p-adaptation.

Thus, we need to find a way to estimate regularity. which can be by smoothness

We present two ways to estimate its smoothness by  by the decay of coefficients from series expansions, namely for Legendre and Fourier series.

There a ways to find out whether a solution is sufficiently smooth.

For these is to expand the finite element approximation into a series 


\textcite{mavriplis1994} introduced with the Legendre coefficient decay.

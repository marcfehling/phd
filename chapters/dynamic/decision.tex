...
On a family of suitably chosen \h-adaptive meshes, \textcite{babuska1996} found the following upper bounds for the global error relative to the number of \glspl{dof} $n_\text{dofs}$:
\begin{align}
...
\end{align}
where constants $b > 0$ and $C$ are both dependent on $m$, but independent of $n_\text{dofs}$. On suitable \hp-adaptive meshes however, \textcites[Thm.~5.1]{guo1986}[Thm.~2.5.2, Thm.~3.5.1]{babuska1996} predicted an even stronger exponential convergence for elliptic problems if the solution $u$ is sufficiently regular:
\begin{align}
